...
9. Teknisi dapat mengunggah foto hasil pekerjaan atau memperkirakan waktu penyelesaian dengan penjelasan.
...
| Foto hasil pekerjaan atau estimasi waktu | FR-041, FR-012, FR-014, US-018 | Should |
...
| Risiko scope creep | Sedang | Upload foto dan unduhan laporan dapat memperbesar pekerjaan jika tidak dibatasi format dan storage-nya. |
...
| Foto atau estimasi teknisi | Tambah kolom estimasi dan penjelasan; foto sebaiknya disimpan sebagai metadata atau URL jika storage tersedia. |
...
| Ganti teknisi | Endpoint pengajuan dan persetujuan penggantian teknisi. |
| Foto hasil atau estimasi | Endpoint update progress teknisi dengan penjelasan dan metadata foto jika digunakan. |
...
| Ganti teknisi | UI pengajuan penggantian dan UI persetujuan teknisi. |
| Foto hasil atau estimasi | Form update progress teknisi berisi estimasi, penjelasan, dan lampiran opsional. |
...
| Cloudflare free tier | Aman untuk fitur berbasis D1 dan Worker. Upload foto perlu dibatasi karena penyimpanan file dapat membutuhkan R2 atau pendekatan lain. |
...
| Upload foto memperbesar scope teknis | Sedang | Jadikan opsional dan tunda penyimpanan file asli jika belum ada storage. |
...
| Penggantian teknisi membingungkan | Sedang | Definisikan aktor persetujuan dan status menunggu persetujuan. |
...
| FR-041 | Ubah atau tunda sebagian | Estimasi waktu diterima; foto hasil pekerjaan sebaiknya opsional karena risiko storage. |
| FR-042 | Terima | Dapat dibuat sederhana dengan CSV. |
...
## Human Review
Manusia perlu memutuskan:
1. Apakah analisis change request implisit ini diterima sebagai pengganti `change-request.md`.
2. Apakah foto hasil pekerjaan pada `FR-041` ditunda, dibuat sebagai URL/metadata, atau tetap memakai storage file.
3. Format unduhan laporan ringkas pada `FR-042`.
4. Status final yang akan dipakai untuk cancelled, reopened, closed otomatis, dan closed admin.
